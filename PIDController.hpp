...
class PIDController {
public:
    PIDController(float kp = 1, float ki = 1, float kd = 1, float deadband = 1) : kp(kp), ki(ki), kd(kd), deadband(deadband) {}
    ...
    float currEncoderPos(float currentEncoderCount) {
      return currentEncoderCount - encoderOffset;
    }
